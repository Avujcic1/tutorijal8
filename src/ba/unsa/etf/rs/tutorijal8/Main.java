...
public class Main {
    ...
    private static void ispisiAutobuse() {
        ...
        for (int i = 0; i < busses.size(); i++) System.out.println((i+1) + ". " + busses.get(i));
    }

    private static void dodijeliAutobusVozacu(Scanner tok) {
        System.out.println("Odaberite vozača: ");
        for (int i = 0; i < dao.getDrivers().size(); i++) System.out.println((i+1) + ". " + dao.getDrivers().get(i));
        System.out.print("Index: ");
        int driverIndex = tok.nextInt()-1;
        Driver driver = dao.getDrivers().get(driverIndex);
        System.out.println("Odaberite autobus: ");
        for (int i = 0; i < dao.getBusses().size(); i++)  System.out.println((i+1) + ". " + dao.getBusses().get(i));
        System.out.print("Index: ");
        ...
        dao.dodijeliVozacuAutobus(driver, bus, broj);
    }

    private static void dodajVozaca(Scanner tok) {
        Scanner stream = tok;
        String ime = stream.nextLine();
        String prezime = stream.nextLine();
        String jmbg = stream.nextLine();
        LocalDate rodjendan = LocalDate.parse(stream.nextLine(), DateTimeFormatter.ofPattern("d.M.yyyy"));
        LocalDate zaposlen = LocalDate.parse(stream.nextLine(), DateTimeFormatter.ofPattern("d.M.yyyy"));
        dao.addDriver(new Driver(ime, prezime, jmbg, rodjendan, zaposlen));
    }
